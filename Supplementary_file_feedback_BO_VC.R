...
#inp_folder <- args[[1]]
#prefix <- args[[2]]
inp_folder <- "D:/Users/VictorC/stack/Victor/Victor_all/RIL_pop/internship_report_Wouter_Lokhorst/trial"
prefix <- "tomato3"
...
# This step has already been run, and now gives an error because we input a NULL variable
...
clean_otu <- otu_table[1:NROW(otu_table),2:(NCOL(otu_table)-1)]
clean_otus <- data.matrix(clean_otu, rownames.force = NA)
...
points(1:length(sums), rep(5000, as.integer(length(sums))), col=c("black"), type="l", lwd=2)
dev.off()
...
normalized_otu_table <- as.matrix(otu_table)
...
normalized_otu_table_10000 <- normalized_otu_table[,-(which(sums<5000))]
...
# Again, code is dependent on the initial removal of the chloroplast genes
# Split the taxonomy
tax_split <- strsplit(as.character(tax$taxonomy), "; ")
parts_matrix <- matrix(unlist(tax_split), ncol=length(tax_split[[1]]), byrow = TRUE)
split_tax <- cbind(parts_matrix, as.character(tax$id))
...
Averages[[8]] <- aggregate(OTU_Averages, by=list(split_tax[,8]), FUN=sum)


# Add the row names as values
Averages[[9]] <- cbind(rownames(OTU_Averages), OTU_Averages)
...
# This is too specific code. Its not good to wrap the entire analysis into a single function
...
taxon_level <- 8
  ...
  A_updated <- rbind(colnames(ave_tax), as.matrix(ave_tax))
...
# what the heck is tax1? its always givin problems
    write.table(tax, file=tax_file, sep=",", quote=F, col.names=F, row.names=F)
  ...
  for (num in 1:12) {
    ...
    if (length(fctr) > 0) {
      legend("topleft", inset=c(0,0), legend=unique(fctr), pch=19, col=unique(cc), title="Name")
    } else {
      legend("topleft", inset=c(0,0), legend=c("None"), pch=19, col=unique(cc), title="Name")
    }
    dev.off()
  }
